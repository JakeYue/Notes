...
常用参数：
    ...
    services          # 定义所有的 service 信息, services 下面的第一级别的 key 既是一个 service 的名称
        ...
        configs               # 不知道怎么用(使用服务 configs 配置为每个服务赋予相应的访问权限，支持两种不同的语法)
 
        cgroup_parent         # 不知道怎么用(可以为容器选择一个可选的父 cgroup)
 
        container_name        # 指定容器的名称 (等同于 docker run --name 的作用)
 
        credential_spec       # 不知道怎么用(为托管服务账户配置凭据规范，此选项仅适用于 Windows 容器服务)
