...
# 指定监听IP地址，可以多次指定。下面两行跟指定接口的作用类似。
listen-address=172.0.0.1,10.211.55.10
...
#pxe-prompt=pxe-prompt=
pxe-service=X86-64_EFI, "Now in X86-64_EFI (UEFI mode), boot from local", 5
# 根据不同的类型使用不同的菜单，使用示例：
